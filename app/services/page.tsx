...
const BlogList = () => {
    ...
    const fetchBlogs = async () => {
      try {
        const res = await client.getEntries({
          content_type: 'services', // Fetch a larger number for pagination
        });
        setBlogs(res.items);
  ...
  return (
    <>
      ...
      <BreadCrumb pageTitle="Services" pagesub={'Services 😍'} pageTop={'Our'} />
                ...
                <div className="portfolio_block portfolio_layout_2">
                  ...
                  <div className="portfolio_content">
                    <h3 className="portfolio_title">
                      <Link  href={`/services/${blog.fields.slug}`} as={`/services/${blog.fields.slug}`}>
                      {blog.fields.title}
                      </Link>
                    </h3>
                    <ul className="category_list unordered_list">
                    <li><Link href={`/services/${blog.fields.slug}`} as={`/services/${blog.fields.slug}`}> {blog.fields.category}</Link></li>
                    </ul>
                  </div>
                </div>
              </div>
